...
$.extend(Cell.prototype, {
	...
	setAllPossible: function() {
		if (this.given) {
			throw "Given " + this + " cannot have all possible numbers";
		}
		this.possibleNumbers.setAll();
		this.cell.text('');
		this.updateView();
	...
	setNumberPossible: function(num) {
		if (this.given && !this.isNumberPossible(num)) {
			throw "Given " + this + " cannot have possibility " + num + " added";
		}
	...
	setNumberImpossible: function(num) {
		if (!this.possibleNumbers.isSet(num)) {
			return false; // Already impossible
		}
		if (this.given) {
			throw "Given " + this + " cannot have possibility " + num + " eliminated";
		}
	...
	toString: function() {
		return "Cell(" + "ABCDEFGHI".charAt(this.row) + (this.col + 1) + ")";
	}
});
...
$.extend(House.prototype, {
	...
	toString: function() {
		if (this.isRow()) {
			return "Row(" + this.rowStart + ")";
		}
		if (this.isCol()) {
			return "Col(" + this.colStart + ")";
		}
		if (this.isBox()) {
			return "Box(" + Math.floor(this.rowStart / 3) + "," + Math.floor(this.colStart / 3) + ")";
		}
		return "House(" + this.rowStart + "-" + this.rowEnd + "," + this.colStart + "-" + this.colEnd + ")";
	}
});
	...
	solveLockedCandidates: function() {
		...
		this.enumHouses(function(house) {
			house.enumUnknownNumbers(function(num) {
				...
				if (1 === objectCardinality(rows) && !house.isRow()) {
					// This House is not a Row, but all of its candidates
					// for this number are in the same Row within this House.
					// None of the Cells in that Row outside this House
					// can contain that number.
					// This House must be a Box, else we have missed a single.
					if (!house.isBox()) {
						throw "solveLockedCandidates: non-Row containing all " + num + "s should be Box but is " + house;
					}
					for (r in rows) {
				...
				if (1 === objectCardinality(cols) && !house.isCol()) {
					// This House is not a Col, but all of its candidates
					// for this number are in the same Col within this House.
					// None of the Cells in that Col outside this House
					// can contain that number.
					// This House must be a Box, else we have missed a single.
					if (!house.isBox()) {
						console.log(rows, cols, boxes);
						throw "solveLockedCandidates: non-Col containing all " + num + "s should be Box but is " + house;
					}
					for (c in cols) {
				...
				if (1 === objectCardinality(boxes) && !house.isBox()) {
					// This House is not a Box, but all of its candidates
					// for this number are in the same Box within this House.
					// None of the Cells in that Box outside this House
					// can contain that number.
					// The House must be either a Row or a Col.
					for (b in boxes) {
						if (house.isRow()) {
							// Portion of Box above this Row
							(new House(board, +b.charAt(0) * 3, house.rowStart - 1, +b.charAt(1) * 3, +b.charAt(1) * 3 + 2)).enumCells(function(cell) {
								if (cell.setNumberImpossible(num)) {
									// console.log("solveLockedCandidates: " + cell + " cannot be a " + num);
									didWork = true;
								}
							...
							(new House(board, house.rowEnd + 1, +b.charAt(0) * 3 + 2, +b.charAt(1) * 3, +b.charAt(1) * 3 + 2)).enumCells(function(cell) {
								if (cell.setNumberImpossible(num)) {
									// console.log("solveLockedCandidates: " + cell + " cannot be a " + num);
									didWork = true;
								}
							...
							(new House(board, +b.charAt(0) * 3, +b.charAt(0) * 3 + 2, +b.charAt(1) * 3, house.colStart - 1)).enumCells(function(cell) {
								if (cell.setNumberImpossible(num)) {
									// console.log("solveLockedCandidates: " + cell + " cannot be a " + num);
									didWork = true;
								}
							...
							(new House(board, +b.charAt(0) * 3, +b.charAt(0) * 3 + 2, house.colEnd + 1, +b.charAt(1) * 3 + 2)).enumCells(function(cell) {
								if (cell.setNumberImpossible(num)) {
									// console.log("solveLockedCandidates: " + cell + " cannot be a " + num);
									didWork = true;
								}
							...
							throw "solveLockedCandidates: non-Box containing all " + num + "s should be Row or Col but is " + house;
						}
					}
				}
				return true; // Next unknown number
